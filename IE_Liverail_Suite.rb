...
describe "A Regression Test Suite for IE" do
	let(:x) { @browser ||= Watir::Browser.new :ie }
	...
	it "should run an autoplay an ad inside a Modal unit and run to completion" do
		...
		it "should run an autoplay ad inside a Modal unit and skip the ad after 2 seconds" do
			Watir::Wait.until {x.div(:class => "container").present?}
				intercept = x.input(:id => "modal").click
		...
		x.window(:title => "Things You Can Do to be More Stylish - The 11 Habits Of Stylish Men - Esquire").use do
			...
			Watir::Wait.until {x.div(:class => "adgContinue").text === "Continue »"}
				...
				event_data = x.execute_script("return GMData.ADGEventHandler.getEventStack()")
				expect(event_data).to eq ["START", "TAG_LOADED", "UI_LOADED", "AD_REQUEST", "AD_RESPONSE_RECEIVED", "AD_LOADED","AD_START", "UNIT_DISPLAYED", "AD_VIDEO_START", "HOVER_ON_PLAYER", "SKIP", "UNIT_CLOSED"]
		end
	end
	...
	# it "should run a CTP ad inside a Modal unit and skip the ad after 2 seconds" do
	# 	Watir::Wait.until {x.div(:class => "container").present?}
	# 		intercept = x.input(:id => "modal").click
	# 		skip = x.input(:id => "delay_skip").click
	# 		skip_time = x.text_field(:id => "secs").set "2"
	...
	it "should load a CTP ad inside a Modal unit and skip the ad before playback begins" do
		...
		Watir::Wait.until {x.div(:class => "adgContinue").text === "Continue »"}
			...
			event_data = x.execute_script("return GMData.ADGEventHandler.getEventStack()")
			expect(event_data).to eq ["START", "TAG_LOADED", "UI_LOADED", "AD_REQUEST", "AD_RESPONSE_RECEIVED", "AD_LOADED", "AD_START", "UNIT_DISPLAYED", "HOVER_ON_PLAYER", "SKIP", "UNIT_CLOSED"]
		end
	end
	...
	it "should run an autoplay ad inside a Locking unit and skip the ad after 2 seconds" do
	...
	# it "should lock the page, load a CTP unit, and skip the ad after 2 seconds" do
	# 	Watir::Wait.until {x.div(:class => "container").exists?}
	# 		intercept = x.input(:id => "locking").click
	# 		skip = x.input(:id => "delay_skip").click
	# 		skip_time = x.text_field(:id => "secs").set "2"
